...
export function parseRow(chunk, { info: { name, group }, currentLine, fields, headers, primaryKeys, exportMode }) {
  let data = chunk.toString('binary').split('\u0001')
  if (data.length !== headers.length) {
    // not ideal, but this takes the first few fields, and the last fields. this is due to bad field-data (they include \1)
    // it will probly have an incomplete description
    if ( ['application'].includes(name)) {
      data =[...data.slice(0, 14), ...data.slice(-3)]
    }
    if ( ['application_detail'].includes(name)) {
      data = [...data.slice(0, 5), null, ...data.slice(-18)]
    }
  }
  ...
  _transform(chunk, encoding, callback) {
    ...
    if (this.currentLine === 0) {
      ...
      if (row) {
        ...
        this.emit('skipped', { chunk, row, lineNumber: this.currentLine })
      }
    }
    callback()
  }
